...
class ContractController extends Controller
{
    ...
    {
        $data = Contract::query()->paginate('10');
        $nationalities = Nationality::all();
        $job_titles = JobTitle::all();
        $applications = Application::all();
        return view('supervisor.contracts.index', compact('data', 'applications', 'nationalities', 'job_titles'));
    }
...
//        $job_titles = JobTitle::all();
//        $applications = Application::all();
//        return view('supervisor.contracts.create', compact('nationalities', 'applications', 'job_titles'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $message = "تم انشاء عقد عمل خاص بك وللاطلاع عليه
برجاء الدخول على لوحة تحكم الموظفين
من خلال الرابط التالى واستخدام
البيانات المرفقة فى عملية تسجيل الدخول
" . $url . "
رقم الجوال هو " . $phone_number . "
وكلمة المرور هى ". $password_clear ;
        ...
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_added'));
    }

    public function show($id)
    {
        $contract = Contract::findorfail($id);
        return view('supervisor.contracts.show', compact('contract'));
    }
    ...
    public function edit($id)
    {
        ...
        $job_titles = JobTitle::all();
        $applications = Application::all();
        return view('supervisor.contracts.edit', compact('contract', 'applications', 'nationalities', 'job_titles'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_updated'));
    }

    public function destroy(Request $request)
    {
        Contract::findOrFail($request->contract_id)->delete();
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_deleted'));
    }

    public function remove_selected(Request $request)
    {
        $contracts_id = $request->contracts;
    ...
    public function print_selected()
    {
        ...
        return view('supervisor.contracts.print', compact('contracts'));
    }

    public function export_contracts_excel()
    {
        return Excel::download(new ContractsExport(), 'كل عقود العمل .xlsx');
    }

    public function show_contract_components(Request $request)
    {
    ...
    public function send_contract_sms($id)
    {
        $contract = Contract::FindOrFail($id);
        $phone_number = $contract->phone_number;
        $employee = $contract->employee;
        $password_clear = $employee->password_clear;
        $url = route('employee.login');
        $message = "تم انشاء عقد عمل خاص بك وللاطلاع عليه
برجاء الدخول على لوحة تحكم الموظفين
من خلال الرابط التالى واستخدام
...
وكلمة المرور هى ".$password_clear;
        ...
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_sent'));
    }
...
وكلمة المرور هى ".$password_clear;
    ...
    public function contract_pending($id)
    {
        $contract = Contract::FindOrFail($id);
        $contract->update([
            'status' => 'pending'
        ...
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_pending'));
    }

    public function contract_declined($id)
    {
        $contract = Contract::FindOrFail($id);
        $contract->update([
            'status' => 'declined'
        ...
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_declined'));
    }

    public function contract_expired($id)
    {
        $contract = Contract::FindOrFail($id);
        $contract->update([
            'status' => 'expired'
        ...
        return redirect()->route('supervisor.contracts.index')
            ->with('success', trans('main.contract_expired'));
    }
...
//        $lg['a_meta_dir'] = 'rtl';
    ...
    public function renew(Request $request)
    {
        ...
        return redirect()->route('supervisor.contracts.index')->with('success',trans('main.contract_renewed'));
    }
}
